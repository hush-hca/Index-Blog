...
import { logout, submitNaverPost } from "@/app/actions";
...
export function PostSubmitLanding({
  ...
  function onSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) {
        setError(result.error ?? "지금은 글을 처리할 수 없습니다.");
        return;
      }

      formRef.current?.reset();
      setMessage(result.message ?? "글 등록 요청이 완료되었습니다.");
    });
  }
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          ...
          <nav className="flex items-center gap-4 text-sm">
            ...
            <button className="text-slate-600 hover:text-slate-950" onClick={scrollToForm} type="button">
              글 등록
            </button>
            {initialAuthenticated ? (
              <>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_420px] lg:items-center">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
              구글 노출을 원하는 네이버 블로그 포스트 주소를 입력하세요. 시스템이 모바일 최적화 주소로 변환해 접수합니다.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-950 p-4 text-white shadow-sm">
            ...
            <div className="mt-5 rounded-md bg-white p-4 text-slate-950">
              ...
              <p className="mt-3 rounded bg-emerald-50 p-3 text-sm text-emerald-800">
                검색 결과에서 내 블로그 글이 노출되는지 직접 확인할 수 있습니다.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[360px_1fr]">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <InfoBlock
              ...
              body="키워드 맞추고 이미지를 직접 찍어가며 포스팅을 100개 넘게 썼는데, 왜 유입은 네이버 포털에만 갇혀 있을까요?"
            />
            <InfoBlock
              title="The Technical Reality"
              body="네이버 블로그는 내부적으로 iframe 구조를 사용하는 폐쇄적인 플랫폼입니다. 이 때문에 구글의 검색 로봇이 글의 실제 텍스트 내용을 스스로 발견하고 읽어가기가 어렵습니다."
            />
            <InfoBlock
              title="The Friction"
              body="일반인이 구글 서치 콘솔 인증 파일을 심거나, 외부 사이트에 색인 신호를 일일이 만드는 것은 너무 머리 아프고 번거로운 일입니다."
            />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <ValueCard
              number="01"
              title="압도적 간편함"
              body='다른 플랫폼에 가입하거나 글을 복사 붙여넣기 하는 작업이 필요 없습니다. 오직 네이버 블로그 "링크(URL)" 하나만 전달하면 끝납니다.'
            />
            <ValueCard
              number="02"
              title="실시간 자동화"
            ...
            <ValueCard
              number="03"
              title="투명한 검증"
              body="작업이 완료된 후 구글 검색창에서 내 글이 정상적으로 색인되었는지 누구나 직접 확인할 수 있습니다."
            />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid gap-4 md:grid-cols-2">
          <MetricCard
            ...
            body="네이버의 잦은 검색 로직 변경과 저품질 리스크에만 의존하지 않고, 안정적인 구글 검색 유입 채널을 함께 확보합니다."
          />
          <MetricCard
            title="장기적인 성장 가능성"
            metric="Long-term Growth"
            body="구글 검색을 통한 정보 탐색형 사용자의 유입으로 블로그 글의 활용 기간과 비즈니스 전환 가능성을 높입니다."
          />
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            <StepCard
              step="Step 1"
              title="URL 접수"
              body="구글 노출을 원하는 네이버 블로그 포스팅 링크를 입력창에 등록합니다. 시스템 내부에서 크롤링 최적화 주소로 자동 변환합니다."
            />
            <StepCard
              step="Step 2"
              title="색인 신호 생성"
              body="검증된 웹 아키텍처를 활용해 Googlebot이 글을 발견할 수 있는 실시간 색인 신호를 만듭니다."
            />
            <StepCard
              step="Step 3"
              title="결과 확인"
              body="색인 등록 후 Googlebot이 실제 네이버 서버를 읽어가기까지 약 7일 정도 소요될 수 있습니다. 이후 검색 연산자로 직접 확인할 수 있습니다."
            />
          </div>
          ...
          <div className="mt-8 rounded-lg border border-slate-200 bg-slate-50 p-6">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <InfoBlock
                title="site: 연산자 검색법"
                body="구글 검색창에 site:https://m.blog.naver.com/본인ID/글번호 형태로 검색해 보세요. 정상 등록됐다면 내 글 제목이 검색 결과에 노출됩니다."
              />
              <InfoBlock
                title="제목 큰따옴표 검색법"
                body={'"네이버 블로그 포스팅 글 제목 전체"를 큰따옴표로 감싸 검색했을 때 내 블로그가 검색 결과에 등장한다면 색인된 상태로 볼 수 있습니다.'}
              />
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-16 md:grid-cols-[1fr_360px] md:items-center">
        <div>
          ...
          <p className="mt-4 leading-7 text-slate-600">
            이 서비스는 개발자 개인이 블로거분들의 고충에 공감해 서버비를 충당하며 운영하고 있습니다.
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <InfoBlock
              title="Q. 내 블로그가 상위 노출되는 것을 보장하나요?"
              body="본 서비스는 구글 검색 결과에 내 글이 등장할 수 있도록 돕는 색인 보조 서비스입니다. 특정 키워드의 상위 노출 순위를 인위적으로 보장하거나 제어하지 않습니다."
            />
            <InfoBlock
              title="Q. 네이버 블로그 저품질이나 패널티 위험은 없나요?"
              body="네이버 블로그 본문이나 코드를 수정하는 방식이 아닙니다. 공개 링크를 따라 검색 로봇이 자연스럽게 방문하도록 만드는 표준 웹 구조를 사용합니다."
            />
          </div>
        </div>
      </section>
